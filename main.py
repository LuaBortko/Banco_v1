...
def salvar():
    arquivo_banco = open("banco.txt", "w")
    for elementos in salva:
        #arquivo_banco.write(str(elementos))
        #arquivo_banco.write("")
        arquivo_banco.write(f"{elementos[0]}#{elementos[1]}#{elementos[2]}#{elementos[3]}#{elementos[4]}#{elementos[5]}\n")
    arquivo_banco.close()

def ler():
    arquivo_banco = open("banco.txt", "r")
    for linha in arquivo_banco.readlines(): #lendo o arquivo e mantendo em x
        if linha: #verifica se tem algo na lista
            linha = linha.split("#") #separa pelo espaco
            linha[3] = float(linha[3])
            linha[5] = linha[5].split(",") #split extrato na virgula
            salva.append(linha)
    arquivo_banco.close()
...
def cadastroclientes():#8. operação livre
    quantidade=int(input("Quantos clientes você deseja cadastrar? "))
